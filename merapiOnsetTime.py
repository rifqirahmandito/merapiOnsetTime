...
if dir == 'mp':
  ...
  # LTE/STE
    ...
    for i in range(len(MPlist)):
      a = read('../mseed/mp/' + MPlist[i])[0]
      sRate = a.stats.sampling_rate
      triggerFunc(a.taper(0.5, type='flattop', side='left'), lte_ste(a, int(9.0*sRate), int(2.5*sRate))[0], np.max(lte_ste(a, int(9.0*sRate), int(2.5*sRate))), str(i+1) + '-LTE/STE', 'mp/lte-ste/MP-LTE-STE-' + str(i+1))
